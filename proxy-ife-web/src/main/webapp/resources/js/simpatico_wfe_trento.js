...
$( function() {
    ...
    var arr = base.split("/");
    var redirect = 'https://dev.smartcommunitylab.it/ife/callback';
    ...
    var authority = 'adc';
        ...
        window.addEventListener('message', function (event) {
        	$.ajax({
                url: 'https://dev.smartcommunitylab.it/ife/userapi/profile',
                type: 'GET',
                dataType: 'json',
...
function definizioni(source, target) {
  //var value = document.getElementById(source).innerText;
	...
	var url = "https://dev.smartcommunitylab.it/ife/api/proxy/textenrich?lex=0&text=" + value;
...
function semplificazione(source, target) {
  //var value = document.getElementById(source).innerText;
	var value = source.replace(/[\t\r\n]/g, '');
	var url = "https://dev.smartcommunitylab.it/ife/api/proxy/textenrich?lex=1&text=" + value;
  //$.getJSON('http://hlt-services7.fbk.eu:8011/simp?text=['+value+']')
	$.getJSON(url)
	  .done(function(json) {
	    //console.log(JSON.stringify(baconGoodness));
...
function wikipedia(source, target) {
	var value = source;
	var url = "https://dev.smartcommunitylab.it/ife/api/proxy/wikipedia?content=" + value;
	$.getJSON(url)
	  .done(function(json) {
	    //console.log(JSON.stringify(baconGoodness));
	  	if(json.error) {
...
function loadModel() {
	var url = "https://dev.smartcommunitylab.it/ife/model.json";
	$.getJSON(url)
  .done(function(json) {
  	workflowModel = json;
